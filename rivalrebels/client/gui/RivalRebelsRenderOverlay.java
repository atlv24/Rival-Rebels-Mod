	...
	private void renderBinoculars(ItemStack item, RenderGameOverlayEvent event, EntityPlayer player)
	{
		if (Mouse.isButtonDown(1))
		{
			...
			if (event.type == ElementType.HOTBAR)
			{
				...
				if (ItemBinoculars.tooFar) fr.drawString(StatCollector.translateToLocal("RivalRebels.controller.range"), (int) ((w * 0.5) - (fr.getStringWidth(StatCollector.translateToLocal("RivalRebels.controller.range")) / 2f)), (int) (h * 0.90), 0xff0000, false);
				else if (ItemBinoculars.tooClose) fr.drawString(StatCollector.translateToLocal("RivalRebels.nextbattle.no"), (int) ((w * 0.5) - (fr.getStringWidth(StatCollector.translateToLocal("RivalRebels.nextbattle.no")) / 2f)), (int) (h * 0.90), 0xff0000, false);
				//else if (dist2 < 40)
				//{
				...
				//}
				else if (ItemBinoculars.ready) fr.drawString(StatCollector.translateToLocal("RivalRebels.binoculars.target"), (int) ((w * 0.5) - (fr.getStringWidth(StatCollector.translateToLocal("RivalRebels.binoculars.target")) / 2f)), (int) (h * 0.90), 0xff0000, false);
				disp = StatCollector.translateToLocal("RivalRebels.message.use")+" "+StatCollector.translateToLocal("RivalRebels.sneak")+" B-83 x2";
				fr.drawString(disp, (int) ((w * 0.5) - (fr.getStringWidth(disp) / 2f)), (int) (h * 0.95), 0xff0000, false);
				
				if (ItemBinoculars.tasks > 0 && ItemBinoculars.dist < 6f)
				{
					Minecraft.getMinecraft().getTextureManager().bindTexture(RivalRebels.ittaskb83);
					
					GL11.glBlendFunc(GL11.GL_ONE, GL11.GL_ONE_MINUS_SRC_ALPHA);
					float col = 1.0f - ItemBinoculars.dist / 6f;
					GL11.glColor4f(col, col, col, 1.0f);
					t.startDrawingQuads();
					t.addVertexWithUV(w * 0.62, h * 0.85 + 16, -90, 0, 1);
					t.addVertexWithUV(w * 0.62 + 16, h * 0.85 + 16, -90, 1, 1);
					t.addVertexWithUV(w * 0.62 + 16, h * 0.85, -90, 1, 0);
					t.addVertexWithUV(w * 0.62, h * 0.85, -90, 0, 0);
					t.draw();
					disp = "x" + ItemBinoculars.tasks;
					fr.drawString(disp, (int) (w * 0.66), (int) (h * 0.87), 0xffff00, false);
				...
				else if (ItemBinoculars.tasks > 0 && ItemBinoculars.hasLaptop)
				{
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
