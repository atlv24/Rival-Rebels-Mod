...
public class TileEntityLaptop extends TileEntity implements IInventory
{
	...
	public void onGoButtonPressed()
	{
		...
		b2spirit += r;
		if (RivalRebels.freeb83nukes) b2spirit += 10;
		PacketDispatcher.packetsys.sendToAll(new LaptopRefreshPacket(xCoord, yCoord, zCoord, b2spirit));
	}
	...
	public int isReady()
	{
		...
		int yield = 0;
		int yield2 = 0;
		if (getStackInSlot(4) == null) r = false;
		if (getStackInSlot(5) == null) r = false;
		if (getStackInSlot(9) == null) r = false;
		if (getStackInSlot(10) == null) r = false;
		for (int j = 0; j < 3; j++)
		{
			if (getStackInSlot(j + 6) != null) yield++;
			if (getStackInSlot(j + 11) != null) yield2++;
		}
		if (yield != yield2) r = false;
		if (r) return yield;
		else return 0;
	}
